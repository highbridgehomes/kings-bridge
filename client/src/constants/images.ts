import KingsBridgeLogo from "../assets/images/kingsbridge-logo.png";
import KingsBridgeLogo2 from "../assets/images/kingsbridge-logo2.png";
import ArchiDesign from "../assets/images/archi-design.jpg";
import EstateConsultancy from "../assets/images/estate-consultancy.jpeg";
import Person from "../assets/images/person.jpg";
import PropertyManagers from "../assets/images/property-managers.webp";
import HappyCustomersOne from "../assets/images/happ-customers1.png";
import HappyCustomersTwo from "../assets/images/happy-customer2.png";
// clientLogo
import ClientLogo1 from "../assets/images/client-1.png";
import ClientLogo2 from "../assets/images/client-2.png";
import ClientLogo3 from "../assets/images/client-3.png";
import ClientLogo4 from "../assets/images/client-4.png";
import ClientLogo5 from "../assets/images/client-5.png";
// projects
import AwoyayaClubHouse from "../assets/images/awoyaya-clubhouse.jpg";
import Chestnut from "../assets/images/chestnut.jpg";
import GardensEstate from "../assets/images/gardens-estate.jpg";
import Haven from "../assets/images/haven.png";
import Residences from "../assets/images/residences.jpg";
// blog posts
import BlogPost1 from "../assets/images/blogpost1.jpg";
import BlogPost2 from "../assets/images/blogpost2.jpg";
import BlogPost3 from "../assets/images/blogpost3.jpg";

export const IMAGES = {
  kingsBridgeLogo: KingsBridgeLogo,
  kingsBridgeLogo2: KingsBridgeLogo2,
  archiDesign: ArchiDesign,
  estateConsultancy: EstateConsultancy,
  person: Person,
  propertyManagers: PropertyManagers,
  happyCustomersOne: HappyCustomersOne,
  happyCustomersTwo: HappyCustomersTwo,
  clientLogo1: ClientLogo1,
  clientLogo2: ClientLogo2,
  clientLogo3: ClientLogo3,
  clientLogo4: ClientLogo4,
  clientLogo5: ClientLogo5,
  awoyayaClubHouse: AwoyayaClubHouse,
  chestnut: Chestnut,
  gardensEstate: GardensEstate,
  haven: Haven,
  residences: Residences,
  blogPost1: BlogPost1,
  blogPost2: BlogPost2,
  blogPost3: BlogPost3,
};
